...
- When stuck or unsure, ask immediately.
...
- Workspace: `~/apps`. Personal apps: `~/apps/personal`. 3rd-party/OSS: `~/apps/oss`. Work apps: `~/apps/digica`.
...
- All imports at the top of the file; never inline inside functions, methods, or blocks.
...
- GitHub broad reads: prefer `gh` first. Raw `gh api search/* -f ...` needs `--method GET`.
- Pasted GitHub issue/PR: first `git status -sb`; if dirty, stop and ask. Pull only with consent. Never push unless explicitly asked.
...
- `~/apps` has intentional same-repo checkouts. Treat as user-managed, not scratch.
